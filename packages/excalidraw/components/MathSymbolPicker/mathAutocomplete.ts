...
export const getExactMathSymbol = (query: string): MathSymbol | undefined => {
  ...
  return (
    ...
    MATH_SYMBOLS.find((s) => s.latex && s.latex.toLowerCase() === `\\${qLower}`)
  );
};

export const findMathSymbolMatches = (
  query: string,
  maxResults = 5,
): MathSymbol[] => {
  if (!query) {
  ...
  for (const s of MATH_SYMBOLS) {
    ...
    if (s.name === query || s.latex === `\\${query}`) {
      ...
      sNameLower.startsWith(qLower) ||
      (sLatexLower && sLatexLower.startsWith(qLower))
    ) {
      startsWith.push(s);
    } else if (
...
export class MathAutocomplete {
  ...
  public handleInput(): void {
    ...
    if (spaceMatch) {
      ...
      if (exact) {
        ...
        const newCaret =
          replaceStart + exact.char.length + trailingSpace.length;
    ...
    if (queryMatch) {
      ...
      const leadingCharLen =
        queryMatch[0].length - trigger.length - query.length;
  ...
  private renderTooltip(): void {
    ...
    this.matches.forEach((m, idx) => {
      ...
      item.setAttribute(
        ...
        cursor: pointer;
        background: ${
          isSelected ? "var(--color-brand, #6965db)" : "transparent"
        };
  ...
  private positionTooltip(): void {
    ...
    const parentRect = this.editable.parentElement?.getBoundingClientRect() || {
      top: 0,
      left: 0,
    };
