...
- A note can be cut to ruled paper (`element.stickyShape: "square" | "ruled"`, absent meaning `"square"`), chosen under "Paper" in the styles panel. A ruled note draws rules at its label’s line height, sitting on its baselines, and pins the label to the top of the note, so text is written on the lines and stays on them as the auto-fit changes the font size. Rules are drawn on canvas and in SVG/PNG exports.
