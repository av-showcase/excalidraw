...
const createLabelBoxTemplate = (): ShapeTemplate => {
  const rectangle = newElement({
    ...
    width: 160,
    height: 60,
    strokeColor: TEMPLATE_ACCENT_COLOR,
    roundness: { type: 3 },
  });
...
let builtinShapeTemplates: ShapeTemplates | null = null;

/**
 * Built lazily on first use: creating elements consumes the global random
 * seed sequence, so doing it at module load would shift seeds for every
 * element created afterwards (and break deterministic snapshots).
 */
export const getBuiltinShapeTemplates = (): ShapeTemplates => {
  if (!builtinShapeTemplates) {
