...
describe("shape templates data", () => {
  ...
  it("includes built-in templates", () => {
    ...
    expect(templates.length).toBeGreaterThanOrEqual(
      getBuiltinShapeTemplates().length,
    );
    expect(
      templates.some((t) => t.id === getBuiltinShapeTemplates()[0].id),
    ).toBe(true);
  });
  ...
  it("persists user templates to localStorage", () => {
    const userTemplate = {
      ...
      name: "Custom",
      category: "Custom",
      elements: getBuiltinShapeTemplates()[0].elements,
    };
  ...
  it("resolves templates by id", () => {
    const id = getBuiltinShapeTemplates()[0].id;
...
describe("shape templates UI", () => {
  ...
  it("inserts template via drag and drop", async () => {
    const templateId = getBuiltinShapeTemplates()[0].id;
